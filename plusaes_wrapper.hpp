...
public:
    ...
    std::string cbc_encrypt(std::string raw, std::string pass) {
        ...
        const unsigned char* passkey = const_cast<unsigned char*>(reinterpret_cast<const unsigned char*>(pass.data()));

        std::vector<unsigned char> e_raw_data(raw_sz);

        auto res = plusaes::encrypt_cbc(const_cast<unsigned char*>(reinterpret_cast<const unsigned char*>(raw.data())), orig_raw_sz, passkey, pass_sz, &m_iv, &e_raw_data[0], e_raw_data.size(), true);
        if (res) {
            return "";
        }
    ...
    std::string ecb_encrypt(std::string raw, std::string pass) {
        ...
        auto res = plusaes::encrypt_ecb(const_cast<unsigned char*>(reinterpret_cast<const unsigned char*>(raw.data())), orig_raw_sz, passkey, pass_sz, &e_raw_data[0], e_raw_data.size(), true);
        if (res) {
            return "";
        }
    ...
    std::string gcm_encrypt(std::string raw, std::string pass, std::string aad = "") {
        ...
        const unsigned char* passkey = const_cast<unsigned char*>(reinterpret_cast<const unsigned char*>(pass.data()));
        unsigned char* aadata = const_cast<unsigned char*>(reinterpret_cast<const unsigned char*>(aad.data()));

        int aadata_sz = aad.size();

        std::vector<unsigned char> e_raw_data(raw_sz);

        auto res = plusaes::encrypt_gcm(const_cast<unsigned char*>(reinterpret_cast<const unsigned char*>(raw.data())), orig_raw_sz, aadata, aadata_sz, passkey, pass_sz, &m_tw_iv, &m_tag);
        if (res) {
            return "";
        }
